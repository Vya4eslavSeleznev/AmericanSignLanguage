...
print(f"Validating on {len(x_test)} images")
...
train_data = ASLImageDataset(x_train, y_train)
test_data = ASLImageDataset(x_test, y_test)
...
def validate(model, dataloader):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader), total=int(len(test_data) / dataloader.batch_size)):
            ...
            running_correct += (predicts == target).sum().item()
